...
export type SettingKey =
  | 'setup_complete' | 'store_name' | 'time_zone' | 'stripe_webhook_secret'
  | 'payment_methods_disabled' | 'cart_enabled' | 'buy_now_enabled' | 'search_provider'
  | 'discounts_enabled' | 'tax_enabled' | 'accounts_enabled' | 'image_optimize'
  | 'image_delivery' | 'shipping_enabled' | 'shipping_config' | 'weight_unit'
...
export interface StoreSettings {
  setupComplete: boolean; storeName: string | null; timeZone: string | null; stripeWebhookSecret: string | null;
...
export async function setSetting(db: D1Database, key: SettingKey, value: string | null | undefined): Promise<void> {
  if (value == null || value === '') { await db.prepare('DELETE FROM settings WHERE key = ?').bind(key).run(); return; }
  await db.prepare(`INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = datetime('now')`).bind(key, value).run();
}
...
export async function setPaymentMethodDisabled(db: D1Database, method: string, disabled: boolean): Promise<void> {
  if (method === 'demo') return;
  const current = (await getSetting(db, 'payment_methods_disabled')) ?? '';
  const set = new Set(current.split(',').map((s) => s.trim()).filter((s) => s && s !== 'demo'));
  if (disabled) set.add(method); else set.delete(method);
