...
export async function createProduct(db: D1Database, p: ProductInput): Promise<number> { return withPublicId('product', async (publicId) => { const row = await db.prepare(`INSERT INTO products (name, slug, description, price_cents, currency, image_key, stock, active, weight_grams, requires_shipping, collect_email, collect_name, collect_virtual_region, public_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`).bind(p.name, p.slug, p.description, p.price_cents, p.currency, p.image_key, p.stock, p.active, p.weight_grams, p.requires_shipping, p.collect_email, p.collect_name, p.collect_virtual_region, publicId).first<{ id: number }>(); return row!.id; }); }
export async function updateProduct(db: D1Database, id: number, p: ProductInput): Promise<void> { await db.prepare(`UPDATE products SET name = ?, slug = ?, description = ?, price_cents = ?, currency = ?, image_key = ?, stock = ?, active = ?, weight_grams = ?, requires_shipping = ?, collect_email = ?, collect_name = ?, collect_virtual_region = ? WHERE id = ?`).bind(p.name, p.slug, p.description, p.price_cents, p.currency, p.image_key, p.stock, p.active, p.weight_grams, p.requires_shipping, p.collect_email, p.collect_name, p.collect_virtual_region, id).run(); }
