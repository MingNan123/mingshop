...
export interface Product {
  ...
  display_stock?: number; display_sold?: number;
  sold?: number;
}
export interface ProductFields { name: string; description: string | null; price_cents: number; currency: string; stock: number; display_stock: number; display_sold: number; active: number; weight_grams: number | null; requires_shipping: number; collect_email: number; collect_name: number; collect_virtual_region: number; }
...
export async function getProductByPublicId(db: D1Database, publicId: string): Promise<Product | null> { return db.prepare(`SELECT p.*, COALESCE((SELECT SUM(oi.quantity) FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE oi.product_id = p.id AND o.status = 'paid'), 0) AS sold FROM products p WHERE p.public_id = ?`).bind(publicId).first<Product>(); }
